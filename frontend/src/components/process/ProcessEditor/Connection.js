  ...
  if (sourceMP.right.x < targetMP.left.x) {
    area = {
      start: {
        x: sourceMP.right.x,
        y:
          sourceMP.right.y < targetMP.left.y
            ? sourceMP.right.y
            : targetMP.left.y,
      },
      end: {
      ...
      arrangement:
        sourceMP.right.y < targetMP.left.y ? ARRANGEMENT_RLT : ARRANGEMENT_RLB,
    }
  }

  if (
    sourceMP.right.x >= targetMP.left.x &&
    sourceMP.bottom.y < targetMP.top.y
  ) {
    area = {
      start: {
        x:
          sourceMP.bottom.x < targetMP.top.x
            ? sourceMP.bottom.x
            : targetMP.top.x,
        y: sourceMP.bottom.y,
      },
      end: {
        x: sourceMP.bottom.x < targetMP.top.x ? targetMP.top.x : sourceMP.top.x,
        y: targetMP.top.y,
      },
      arrangement:
        sourceMP.bottom.x < targetMP.top.x ? ARRANGEMENT_BTL : ARRANGEMENT_BTR,
    }
  }
  ...
  if (sourceMP.left.x > targetMP.right.x) {
    area = {
      start: {
        x: targetMP.right.x,
        y:
          sourceMP.left.y < targetMP.right.y
            ? sourceMP.left.y
            : targetMP.right.y,
      },
      end: {
      ...
      arrangement:
        sourceMP.right.y < targetMP.left.y ? ARRANGEMENT_LRT : ARRANGEMENT_LRB,
    }
  }

  if (
    sourceMP.top.y >= targetMP.bottom.y &&
    sourceMP.right.x > targetMP.left.x &&
    sourceMP.left.x < targetMP.right.x
  ) {
    area = {
      start: {
        x:
          targetMP.bottom.x < sourceMP.top.x
            ? targetMP.bottom.x
            : sourceMP.top.x,
        y: targetMP.bottom.y,
      },
      ...
      arrangement:
        sourceMP.bottom.x < targetMP.top.x ? ARRANGEMENT_TBL : ARRANGEMENT_TBR,
    }
  }
