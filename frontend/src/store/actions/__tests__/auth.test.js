...
describe('auth action', () => {
  it('auth success', () => {
    ...
    return store.dispatch(actions.auth('John', 'pass')).then(() => {
  ...
  it('auth failure', () => {
    ...
    return store.dispatch(actions.auth('Silver', 'pass')).then(() => {
      expect(store.getActions()).toEqual(expectedActions)
    })
  })
})
...
describe('refresh auth token action', () => {
  it('success', () => {
    ...
    return store.dispatch(actions.refreshAuthToken()).then(() => {
      expect(store.getActions()).toEqual(expectedActions)
    })
  })
  ...
  it('failure', () => {
    const store = mockStore({})
    const expectedActions = [{ type: AUTH_REQUEST }, { type: LOGOUT }]
